...
import sunrisedds_interfaces.msg.SmartServo;

public class SmartServoPubSub {

    private static final Logger logger = LoggerFactory.getLogger(SmartServoPubSub.class);
    ...
    public static void main(String[] args) {
        try {
            ...
            Topic<SmartServo> topic = participant.createTopic(SmartServo.class, "rt/smartservo");
            DataWriter<SmartServo> writer = publisher.createDataWriter(topic);
            DataReader<SmartServo> reader = subscriber.createDataReader(topic);
            ...
            SmartServo message = new SmartServo();

            message.setType(SmartServo.SMART_SERVO_JOINT_POSITION_VELOCITY);
            message.getJointPosition().setA1(1).setA2(2).setA3(3).setA4(4).setA5(5).setA6(6).setA7(7);
            message.getJointVelocity().setA1(1).setA2(2).setA3(3).setA4(4).setA5(5).setA6(6).setA7(7);

            writer.write(message);
            logger.info("Wrote message");

            SmartServo read_message = reader.read();
